...
from utilities.ErrorHandlers import UnexpectedArgumentHandler
...
def create_app(test_config=None):
    ...
    @app.route('/questions')
    @UnexpectedArgumentHandler(["page"])
    def get_questions():
        page = 1
    ...
    def search_questions():
        ...
        query_param = Question.question.ilike(search_term)
        total_questions = Question.query.filter(query_param).all()
        page = request.args.get('page', 1, type=int)
        start = (page - 1) * QUESTIONS_PER_PAGE
        end = start + QUESTIONS_PER_PAGE
    ...
    def get_quiz_question():
        ...
        if cat != 0:
            all_questions = Question.query.filter((Question.category == cat))
        else:
            all_questions = Question.query
        query_param = Question.id.notin_(pre_questions)
        all_questions = all_questions.filter(query_param).all()
